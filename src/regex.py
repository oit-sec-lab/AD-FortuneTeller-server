...
def regex_check(url_dict):                  # urlの辞書配列を引数に受け取り
    url_list = crawling.crawling_check(url_dict)    # url配列からサイト内リンクのクローリングを行った結果をリストに格納
    dict = {}
    for i, row in enumerate(url_list):      # 行毎の処理(サイト毎の処理)
